...
public class Player : LivingEntity
{
    ...
    {
        // 트리거 충돌한 상대방 게임 오브젝트가 Enemy이라면 공격 실행
        if (!dead && (collision.tag == "Enemy" || collision.tag == "EnemyTower") && !collision.isTrigger)
        {
            Move(0f);
            ...
            if (Time.time >= lastAttackTime + timeBetAttack)
            {
                ...
                GameObject.Find("SFXSound").GetComponent<SFXSound>().PlaySFX(attackClip);
